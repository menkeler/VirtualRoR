  ...
  const handleSubmit = async (e) => {
    ...
    if (message && datePreferred) {
      ...
      try {
        ...
        const responseInquiry = await client.post(
        ...
        const responseinquiryItems = await client.post(
          "transactions/inquiries_item/",
          cartData,
          {
            headers: {
              Authorization: `Token ${authToken}`,
      ...
      alertDiv.className =
        "fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-green-500 text-white px-4 py-2 rounded-md flex items-center";
                    ...
                    <>
                      ...
                      <input
                        ...
                        onBlur={(e) => {
                          ...
                          // If the entered value exceeds the maximum allowed quantity, reset it to the maximum allowed quantity
                          if (enteredValue > cartItem.maxquantity) {
                            e.target.value = cartItem.maxquantity;
                            // Call the adjustQuantity function with the maximum allowed quantity
                            adjustQuantity(
                              cartItem.item,
                              cartItem.inventory,
      ...
      <div className="md:text-center p-8">
        <div className="max-w-md mx-auto p-6 bg-white rounded-md shadow-md">
          ...
          <button
            ...
            onClick={() =>
              document.getElementById("InquiryReservation").showModal()
            }
          ...
          {cartState.cartItems.length === 0 && (
            <p className="text-red-500 mt-2">
              Add items to the cart to proceed with the reservation.
            </p>
          )}
