...
const DashboardTransactionPage = ({ User }) => {
    ...
    if (!isTransitioning) {
      ...
      setTimeout(() => {
        setActiveComponent((prevComponent) =>
          direction === 'next' ? (prevComponent === 'release' ? 'donation' : 'release') : (prevComponent === 'release' ? 'donation' : 'release')
        );
  ...
  return (
    ...
    <div className="flex items-center justify-center">
      <button disabled={isTransitioning} onClick={() => toggleComponent('prev')} className="mr-4">
      ...
      <button disabled={isTransitioning} onClick={() => toggleComponent('next')} className="ml-4">
