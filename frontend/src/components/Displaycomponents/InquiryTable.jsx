  ...
  const fetchInquiries = async (
    ...
    try {
      ...
      const response = await client.get(
        `transactions/inquiries/?page=${page}&ordering=status&status=${encodeURIComponent(
          status
        )}&type=${encodeURIComponent(
          type
        )}&search=${encodedSearchQuery}&user=${UserId}`
      );
            ...
            {inquiries.map((inquiry) => (
              <React.Fragment key={inquiry.id}>
                <tr
                  ...
                  onClick={() =>
                    document
                      .getElementById(`DetailInquiry${inquiry.id}`)
                      .showModal()
                  }
                >
                  <td className="py-2 px-4 border-b ">{inquiry.id}</td>
                  <td className="py-2 px-4 border-b">
                    {new Date(inquiry.date_created).toLocaleString("en-US", {
                      dateStyle: "medium",
                      timeStyle: "medium",
                    })}
                  </td>
